<?php
/**
 * Class for the import actions used in the PostmagThemes Demo Import plugin.
 * Register default WP actions for PMDI plugin.
 *
 * @package pmdi
 */
...
class ImportActions {
	...
	public function register_hooks() {
		...
		add_action( 'pt-pmdi/after_content_import_execution', array( $this, 'before_widget_import_action' ), 10, 3 );
		add_action( 'pt-pmdi/after_content_import_execution', array( $this, 'widgets_import' ), 20, 3 );

		// Customizer import.
		add_action( 'pt-pmdi/customizer_import_execution', array( $this, 'customizer_import' ), 10, 1 );
		...
		if ( apply_filters( 'pt_pmdi/enable_custom_menu_widget_ids_fix', true ) ) {
			add_action( 'pt-pmdi/widget_settings_array', array( $this, 'fix_custom_menu_widget_ids' ) );
		}
	}
	...
	public function fix_custom_menu_widget_ids( $widget ) {
		...
		$content_import_data = $pmdi->importer->get_importer_data();
	 ...
	 * @param array $import_files          The filtered import files defined in `pt-pmdi/import_files` filter.
	...
	/**
	 * Execute the action: 'pt-pmdi/before_content_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer).
	 * @param array $import_files          The filtered import files defined in `pt-pmdi/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_content_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'pt-pmdi/before_content_import', $import_files[ $selected_index ] );
	}


	/**
	 * Execute the action: 'pt-pmdi/before_widgets_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer).
	 * @param array $import_files          The filtered import files defined in `pt-pmdi/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_widget_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'pt-pmdi/before_widgets_import', $import_files[ $selected_index ] );
	}


	/**
	 * Execute the action: 'pt-pmdi/after_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer).
	 * @param array $import_files          The filtered import files defined in `pt-pmdi/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	 ...
	 * @param array  $selected_import The data of selected import from `pt-pmdi/import_files` filter.
	 */
